...
boolMol=0;
set(handles.ExportRASP,'Enable','off');
...
function loadMol_Callback(hObject, eventdata, handles)
    ...
    global S_all;
    global mean_macroT;
    global mean_microT_Donor;
    global mean_microT_Acceptor;
    global boolMol;
    ...
    % Open bin file
    [fname, pname]=uigetfile('*.bin','Open bin file with filtered bursts');
    fid=fopen([pname fname]);
    
    set(handles.figure1,'Name',['FitXGauss - ' fname(1:end-4)]);

    % Open setting file
    strS='Settings.txt';
    para_Set=importdata([pname strS]);

    % lifetime ROI
    Brd_GR=[para_Set(end,1) para_Set(end,2)];
    Brd_R0=[para_Set(end,3) para_Set(end,4)];

    % Set correction factors
    corrFac={'0';'0';'1';'0.025';'0.0125';'0.016';'0';'0'};
    corrFac=inputdlg({'alpha','beta','gamma','bin size','offset','bin size - lifetime (ns)','IRF shift - donor channel (ns)','IRF shift - acceptor channel (ns)'},'Correction factors',1,corrFac);
    alpha=str2double(corrFac{1});
    beta=str2double(corrFac{2});
    gamma=str2double(corrFac{3});
    binsize=str2double(corrFac{4});
    offset=str2double(corrFac{5});
    binsize_microtime=str2double(corrFac{6});
    shiftIRF_D=str2double(corrFac{7});
    shiftIRF_A=str2double(corrFac{8});
    
    % histogram edges
    edgesFRET=(-0.1+offset:binsize:1.1+offset)';

    % get length of data file
    fseek(fid,0,'eof');
    fLength=ftell(fid)/8/4;
    fseek(fid,0,'bof');

    Data=zeros(10000,4);
    posData=1;
    Data(posData,:)=fread(fid,[1 4],'double');
    currBN=Data(1,1);
    iterM=1;
    ...
    mean_macroT=[];
    mean_microT_Donor=[];
    mean_microT_Acceptor=[];
    molFRET={[]};

    h=waitbar(0,'Loading bursts ..');
    for posFile=2:fLength

        readData=fread(fid,[1 4],'double');

        if readData(1)==currBN

            posData=posData+1;
            Data(posData,:)=readData;

        else

            DPh=Data((Data(1:posData,2)==2|Data(1:posData,2)==4)&Data(1:posData,3)>Brd_GR(1)&Data(1:posData,3)<Brd_GR(2),:);
            APh=Data((Data(1:posData,2)==1|Data(1:posData,2)==3)&Data(1:posData,3)>Brd_GR(1)&Data(1:posData,3)<Brd_GR(2),:);
            A0Ph=Data((Data(1:posData,2)==1|Data(1:posData,2)==3)&Data(1:posData,3)>Brd_R0(1)&Data(1:posData,3)<Brd_R0(2),:);

            NGG=length(DPh(:,1));
            NGR=length(APh(:,1));
            NRR=length(A0Ph(:,1));

            % create cell for all bursts
            if ~(isempty(DPh)||isempty(APh)||isempty(A0Ph)) % only take burst with events
             
                
                % FRET and Stoichiometry
                corrE=(NGR-alpha*NRR-beta*NGG)/(NGR-alpha*NRR-beta*NGG+gamma*NGG);
                corrS=(NGR-alpha*NRR-beta*NGG+gamma*NGG)/(NGR-alpha*NRR-beta*NGG+gamma*NGG+NRR);
                
                hE=histc((NGR-alpha*NRR-beta*NGG)/(NGR-alpha*NRR-beta*NGG+gamma*NGG),edgesFRET);
                
                if sum(hE)~=0
                    
                    E_all(end+1)=corrE;
                    S_all(end+1)=corrS;
                    mean_macroT(end+1)=mean([DPh(:,4);APh(:,4);A0Ph(:,4)]);
                    molFRET{iterM,1}=hE';
                    molFRET{iterM,2}=1;
                    mean_microT_Donor(end+1)=mean(DPh(:,3)-Brd_GR(1)).*binsize_microtime-shiftIRF_D;
                    mean_microT_Acceptor(end+1)=mean(A0Ph(:,3)-Brd_R0(1)).*binsize_microtime-shiftIRF_A;
                    
                    iterM=iterM+1;
                end
            end
                        
            % Reset Data Values and go to new BN
            currBN=readData(1);
            posData=1;
            Data(:,:)=0;
            Data(posData,:)=readData;
        ...
        if mod(posFile,10000)==0
            waitbar(posFile/fLength);
        end
    end
    fclose(fid);
    close(h);
        
    sumFRET=(histc(E_all,edgesFRET))';
    weightFRET=sumFRET;
    numMol=length(E_all);
    ...
    set(handles.numMolecules,'String',[num2str(numMol) ' Molecules']);
    set(handles.popupGauss,'Enable','on');    
    set(handles.editSamples,'String',num2str(numMol));
    set(handles.editIterations,'String',num2str(100));
    set(handles.ExportRASP,'Enable','on');
    boolMol=1;
...
function ExportRASP_Callback(hObject, eventdata, handles)
    ...
    data.E_all=E_all;
    data.S_all=S_all;
    data.para_Fit=results2EXP;
    data.mean_macroT=mean_macroT;
    data.mean_microT_Donor=mean_microT_Donor;
    data.mean_microT_Acceptor=mean_microT_Acceptor;
